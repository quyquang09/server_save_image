...
export default function ParamValidate(req: Req, route: any, prefix: any) {
  ...
  const parseFormDataArray = (body: any) => {
    if (body && Object.keys(body).find((e) => e.includes("[") && e.includes("]"))) {
      const keys = Object.keys(body).filter((e) => e.includes("[") && e.includes("]"));
      keys.map((e) => {
        const k = e.split("[")[0];
        if (!body[k]) body[k] = [];
        body[k].push(body[e]);
  ...
  const dataModel: any = {};
  const requiresJWT = parameters && Object.keys(parameters).find((k: string) => k == "jwt");
  parameters &&
    Object.keys(parameters).map((k: string) => {
      ...
      switch (param.in) {
        case SwaggerInTypes.path:
          {
            ...
            if (param.required && (data == null || data == undefined)) missing(k);
        ...
        default:
          {
            const malformedArray = isArrayType && !!body[k] && !Array.isArray(body[k]);
...
export const validateType = (data: any, param: any) => {
  if (!!param.regex && typeof data === "string" && !(param.regex as RegExp).test(data)) {
    return false;
  }
  switch (param.type) {
    case SwaggerDataTypes.integer:
